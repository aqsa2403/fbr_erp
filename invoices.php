...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] === 'add_invoice') {
        ...
        if ($companyId && !empty($totalValue) && $invoiceId) {
            $stmt = $db->prepare("UPDATE DI_Invoice_Master SET 
                CompanyID = ?, BranchID = ?, InvoiceDate = ?, BuyerNTNCNIC = ?, BuyerBusinessName = ?, TotalInvoiceValue = ? 
                WHERE InvoiceID = ?");
            $stmt->execute([
                $companyId,
                $branchId,
...
<div id="addInvoiceForm"
    class="<?= $editInvoice ? '' : 'hidden ' ?>bg-white p-6 rounded-2xl shadow-sm border border-slate-200 mb-8">
    <div class="flex justify-between items-center mb-4 border-b pb-2">
        <h3 class="font-bold text-slate-700"><?= $editInvoice ? 'Edit Invoice' : 'Create New Invoice Tracker' ?></h3>
        <?php if ($editInvoice): ?>
            <a href="invoices.php" class="text-slate-400 hover:text-red-500"><i class="fas fa-times"></i> Cancel</a>
        <?php endif; ?>
    </div>
    <form action="invoices.php" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="md:col-span-2">
            ...
            <input type="number" step="0.01" name="totalValue" required
                value="<?= $editInvoice ? htmlspecialchars($editInvoice['TotalInvoiceValue']) : '' ?>"
                class="w-full bg-slate-50 border-2 border-slate-100 p-3 rounded-xl outline-none focus:border-blue-500">
        </div>
                    ...
                    <?php foreach ($invoices as $inv): ?>
                        <tr class="hover:bg-slate-50">
                            ...
                            <td class="p-5 text-slate-700">
                                <?= htmlspecialchars(date('Y-m-d', strtotime($inv['InvoiceDate']))) ?>
                            </td>
